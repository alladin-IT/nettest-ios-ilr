...
extension CLLocation: RMBTFormat {
    ...
    func rmbtFormattedString() -> String {
        ...
        var latSeconds = Int(round(abs(self.coordinate.latitude * 3600)))
        let latDegrees = latSeconds / 3600
        latSeconds = latSeconds % 3600
        let latMinutes: CLLocationDegrees = Double(latSeconds) / 60.0

        var longSeconds = Int(round(abs(self.coordinate.longitude * 3600)))
    ...
    func paramsDictionary() -> NSDictionary {
        return [
            "long":     NSNumber(value: coordinate.longitude),
            "lat":      NSNumber(value: coordinate.latitude),
            "time":     RMBTTimestampWithNSDate(timestamp),
            "accuracy": NSNumber(value: horizontalAccuracy),
            "altitude": NSNumber(value: altitude),
            "speed":    NSNumber(value: (speed > 0 ? self.speed : 0.0))
        ]
    }
