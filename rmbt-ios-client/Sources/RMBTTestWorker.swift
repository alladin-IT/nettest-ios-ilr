...
public protocol RMBTTestWorkerDelegate {
    ...
    ///
    func testWorker(_ worker: RMBTTestWorker, didConnectToHost host: String, port: UInt16, localPort: UInt16, inState state: RMBTTestWorkerState)
...
open class RMBTTestWorker: NSObject, GCDAsyncSocketDelegate {
    ...
    fileprivate var testDownloadedData: NSMutableData!
...
// MARK: Socket delegate methods
    ...
    open func socket(_ sock: GCDAsyncSocket, didConnectToHost host: String, port: UInt16) {
        ...
        delegate.testWorker(self, didConnectToHost: host, port: sock.connectedPort, localPort: sock.localPort, inState: state)
        ...
        // Downlink test
            ...
            if chunkData == nil {
                ...
                if testDownloadedData.length >= Int(chunksize) {
                    chunkData = NSData(data: testDownloadedData.subdata(with: NSRange(location: 0, length: Int(chunksize)))) as Data
                }
...
// MARK: Socket helpers
    ...
    fileprivate func updateLastChunkFlagToValue(_ lastChunk: Bool) {
        ...
        chunkData.replaceSubrange(chunkData.count - 1..<chunkData.count, with: Data(bytes: [lastByte])) // TODO: is this correct?
        //chunkData.replaceBytes(in: NSRange(location: chunkData.length - 1, length: 1), withBytes: &lastByte)
    }
}
