...
class QOSUDPTestExecutor<T: QOSUDPTest>: QOSTestExecutorClass<T>, UDPStreamSenderDelegate, UDPStreamReceiverDelegate { /*swift compiler segfault if moved to extension*/
    ...
    override func executeTest() {
        ...
        if testObject.packetCountOut == nil && testObject.packetCountIn == nil {
            testResult.set(RESULT_UDP_NUM_PACKETS_OUTGOING_RESPONSE, string: "NOT_SET")
            testResult.set(RESULT_UDP_NUM_PACKETS_INCOMING_RESPONSE, string: "NOT_SET")
...
// MARK: test methods
    ...
    private func finishOutgoingTest() {
        testResult.set(RESULT_UDP_OUTGOING_PACKETS,                 number: resultPacketData.rcvServerResponse)
        testResult.set(RESULT_UDP_NUM_PACKETS_OUTGOING_RESPONSE,    number: resultPacketData.numPackets)
        ...
        if lostPackets > 0 {
            ...
            testResult.set(RESULT_UDP_OUTGOING_PLR, string: "\(packetLossRate)")
        } else {
            testResult.set(RESULT_UDP_OUTGOING_PLR, string: "0")
        }
...
// MARK: QOSControlConnectionDelegate methods
    ...
    ///
    private func appendPacketData(_ data: inout Data, flag: UInt8, packetNumber: UInt16) {

        // write flag
        data.append(flag) // TODO: is this correct?
        //data.appendValue(flag)

        // write packetNumber
        data.append(UInt8(packetNumber)) // TODO: is this correct? // make sure only 1 byte is used for packageNumber here
...
// MARK: UDPStreamReceiverDelegate methods
    ...
    ///
    func udpStreamReceiver(_ udpStreamReceiver: UDPStreamReceiver, willSendPacketWithNumber packetNumber: UInt16, data: inout Data) -> Bool {
