...
open class QualityOfServiceTest {
    ...
    ///
    fileprivate let executorQueue = DispatchQueue(label: "at.alladin.rmbt.executorQueue", attributes: DispatchQueue.Attributes.concurrent)

    ///
    fileprivate let qosQueue = DispatchQueue(label: "at.alladin.rmbt.qosQueue", attributes: DispatchQueue.Attributes.concurrent)

    ///
    fileprivate let mutualExclusionQueue = DispatchQueue(label: "at.alladin.rmbt.qos.mutualExclusionQueue", attributes: [])
    ...
    ///
    init(controlServer: ControlServer, testToken: String, measurementUuid: String, speedtestStartTime: UInt64) {
    ...
    fileprivate func fetchQOSTestParameters() {
        ...
        // TODO: remove controlServer dependency, give parameters when creating a QualityOfServerTest object
            ...
            if let testArray = qosTestConcurrencyGroupMap[concurrencyGroup] {
                ...
                for qosTest in testArray {
                    ...
                    if let testExecutor = QOSFactory.createTestExecutor(qosTest, delegateQueue: executorQueue, speedtestStartTime: speedtestStartTime) {
                        ...
                        // check for control connection
                        if qosTest.needsControlConnection() {
                            logger.debug("qosTest \(qosTest.getType().rawValue) \(qosTest.qosTestId) needs control connection")
                            ...
                            if let controlConnection = getControlConnection(qosTest), controlConnection.connected { // blocking if new connection has to be established
                                ...
                                testExecutor.setControlConnection(cc: controlConnection)
                            } else {
                                // don't do this test
    ...
    fileprivate func getControlConnection(_ qosTest: QOSTest) -> QOSControlConnection? {
        guard let serverAddress = qosTest.serverAddress, let serverPort = qosTest.serverPort else {
            return nil
        }

        // determine control connection
        let controlConnectionKey: String = "\(serverAddress)_\(serverPort)"
        ...
        if conn == nil {
            ...
            // connect
            /* let isConnected = */_ = conn.connect(serverAddress, onPort: serverPort) // blocking
        ...
        if !conn.connected {
            // reconnect
            _ = conn.connect(serverAddress, onPort: serverPort)
        }
    ...
    fileprivate func submitQOSTestResults() {
        ...
        // TODO: remove controlServer dependency, just return the results, other class should handle submit
