...
//    let TIMEOUT_EXTENSION: UInt64 = /*1*/1000 * NSEC_PER_MSEC // add 100ms to timeout because of compution etc.
    ...
    ///
    private(set) var controlConnection: QOSControlConnection?
...
//    private let timeoutCountDownLatch: CountDownLatch = CountDownLatch()
    ...
    ///
    init(delegateQueue: DispatchQueue, testObject: T, speedtestStartTime: UInt64) {
        ...
        testResult.set(RESULT_TEST_UID, number: self.testObject.qosTestId)
    ...
    ///
    func setControlConnection(cc controlConnection: QOSControlConnection?) {
    ...
    ///
    internal func execute(finish finishCallback: @escaping (QOSTestResult) -> ()) {
...
//        timeoutCountDownLatch.countDown()
        ...
        // TODO: IMPROVE...let tests don't do anything if they are finished!
...
class QOSLog {
    ...
    ///
    func verbose(_ logMessage: String, functionName: StaticString = #function, fileName: StaticString = #file, lineNumber: Int = #line) {
        self.logln(logMessage, logLevel: .verbose, functionName: functionName, fileName: fileName, lineNumber: lineNumber)
    }

    ///
    func debug(_ logMessage: String, functionName: StaticString = #function, fileName: StaticString = #file, lineNumber: Int = #line) {
        self.logln(logMessage, logLevel: .debug, functionName: functionName, fileName: fileName, lineNumber: lineNumber)
    }

    ///
    func info(_ logMessage: String, functionName: StaticString = #function, fileName: StaticString = #file, lineNumber: Int = #line) {
        self.logln(logMessage, logLevel: .info, functionName: functionName, fileName: fileName, lineNumber: lineNumber)
    }

    ///
    func warning(_ logMessage: String, functionName: StaticString = #function, fileName: StaticString = #file, lineNumber: Int = #line) {
        self.logln(logMessage, logLevel: .warning, functionName: functionName, fileName: fileName, lineNumber: lineNumber)
    }

    ///
    func error(_ logMessage: String, functionName: StaticString = #function, fileName: StaticString = #file, lineNumber: Int = #line) {
        self.logln(logMessage, logLevel: .error, functionName: functionName, fileName: fileName, lineNumber: lineNumber)
    }

    ///
    func severe(_ logMessage: String, functionName: StaticString = #function, fileName: StaticString = #file, lineNumber: Int = #line) {
        self.logln(logMessage, logLevel: .severe, functionName: functionName, fileName: fileName, lineNumber: lineNumber)
    }

    ///
    func logln(_ logMessage: String, logLevel: XCGLogger.Level = .debug, functionName: StaticString = #function, fileName: StaticString = #file, lineNumber: Int = #line) {
        if QOS_ENABLED_TESTS_LOG.contains(testType) {
            let message = "\(testType.rawValue.uppercased())<\(testUid)>: \(logMessage)"

            logger.logln(message, level: logLevel, functionName: functionName, fileName: fileName, lineNumber: lineNumber)
        }
    }
}
