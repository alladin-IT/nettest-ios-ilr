...
public protocol RMBTTestRunnerDelegate {
    ...
    ///
    func testRunnerDidMeasureThroughputs(_ throughputs: [RMBTThroughput]? /* TODO: ? */, inPhase phase: RMBTTestRunnerPhase)
...
class RMBTTestRunner: NSObject, RMBTTestWorkerDelegate, RMBTConnectivityTrackerDelegate {
    ...
    ///
    init(controlServer: ControlServer, delegate: RMBTTestRunnerDelegate) {
    ...
    func start() {
        ...
        // TODO: remove controlServer dependency, create RMBTTestRunner with parameters
...
// MARK: Test worker delegate method
    ...
    ///
    open func testWorker(_ worker: RMBTTestWorker, didConnectToHost host: String, port: UInt16, localPort: UInt16, inState state: RMBTTestWorkerState) {
        
        var direction: SpeedMeasurementSourcePortMap.Direction = .down
        if (state == .uplinkPretestStarted) {
            direction = .up
        }
        
       speedMeasurementResult.setSourcePort(port: Int(localPort), direction: direction, forThreadIndex: Int(worker.index))
    }
...
///////
    ...
    fileprivate func submitResult() {
        workerQueue.async {
            ...
            // TODO: remove controlServer dependency, let other class submit results
            self.controlServer.submitSpeedMeasurementResult(speedMeasurementResultRequest, success: { response in
                self.workerQueue.async {
                    //self.phase = .None
                    self.setPhase(.none)
                    self.dead = true
    ...
    fileprivate func resultObject() -> SpeedMeasurementResult {
        ...
        if let firstWorker = workers.first {
            speedMeasurementResult.totalBytesDownload = sumBytesDownloaded // TODO: ?
            speedMeasurementResult.totalBytesUpload = sumBytesUploaded // TODO: ?
        ...
        if let startInfo = startInterfaceInfo, let uplinkEndInfo = uplinkEndInterfaceInfo, startInfo.bytesReceived <= uplinkEndInfo.bytesReceived && startInfo.bytesSent < uplinkEndInfo.bytesSent {
            speedMeasurementResult.interfaceTotalBytesDownload = UInt64(uplinkEndInfo.bytesReceived) - UInt64(startInfo.bytesReceived)
            speedMeasurementResult.interfaceTotalBytesUpload = UInt64(uplinkEndInfo.bytesSent) - UInt64(startInfo.bytesSent)
        }
        ...
        if let downStartInfo = downlinkStartInterfaceInfo, let downEndInfo = downlinkEndInterfaceInfo, downStartInfo.bytesReceived <= downEndInfo.bytesReceived && downStartInfo.bytesSent < downEndInfo.bytesSent {
            speedMeasurementResult.interfaceDltestBytesDownload = UInt64(downEndInfo.bytesReceived) - UInt64(downStartInfo.bytesReceived)
            speedMeasurementResult.interfaceDltestBytesUpload = UInt64(downEndInfo.bytesSent) - UInt64(downStartInfo.bytesSent)
        }
        ...
        if let upStartInfo = uplinkStartInterfaceInfo, let upEndInfo = uplinkEndInterfaceInfo, upStartInfo.bytesReceived <= upEndInfo.bytesReceived && upStartInfo.bytesSent < upEndInfo.bytesSent {
            speedMeasurementResult.interfaceUltestBytesDownload = UInt64(upEndInfo.bytesReceived) - UInt64(upStartInfo.bytesReceived)
            speedMeasurementResult.interfaceUltestBytesUpload = UInt64(upEndInfo.bytesSent) - UInt64(upStartInfo.bytesSent)
        }
        ...
        speedMeasurementResult.relativeTimeDlNs = downlinkTestStartedAtNanos - startNanos
        speedMeasurementResult.relativeTimeUlNs = uplinkTestStartedAtNanos - startNanos
...
// MARK: Utility methods
    ...
    fileprivate func startPhase(_ phase: RMBTTestRunnerPhase, withAllWorkers allWorkers: Bool, performingSelector selector: Selector!,
        ...
        if duration > 0 {
            ...
            timer = DispatchSource.makeTimerSource(flags: DispatchSource.TimerFlags(rawValue: 0), queue: DispatchQueue.global(qos: DispatchQoS.QoSClass.default))
            ...
            /*timer.scheduleRepeating(deadline: DispatchTime.now(), interval: DispatchTimeInterval.nanoseconds(Int(RMBTTestRunnerProgressUpdateInterval * Double(NSEC_PER_SEC))), leeway: DispatchTimeInterval.seconds(50)) // TODO: is this correct?
             */
...
// MARK: Public API
    ...
    ///
    open func downloadKilobitsPerSecond() -> UInt64 {
        return speedMeasurementResult.totalDownloadHistory.totalThroughput.kilobitsPerSecond()
    }

    ///
    open func uploadKilobitsPerSecond() -> UInt64 {
        return speedMeasurementResult.totalUploadHistory.totalThroughput.kilobitsPerSecond()
    }
