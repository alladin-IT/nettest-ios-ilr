...
class QOSTCPTestExecutor<T: QOSTCPTest>: QOSTestExecutorClass<T>, GCDAsyncSocketDelegate {
    ...
    ///
    private let socketQueue = DispatchQueue(label: "at.alladin.rmbt.tcp.socketQueue"/*, attributes: DispatchQueue.Attributes.concurrent*/)
    ...
    override func executeTest() {
        ...
        if let portOut = testObject.portOut {
            ...
            testResult.set(RESULT_TCP_OUT, string: "FAILED") // assume test failed if port was provided (don't have to set failed later, only have to success)
        ...
        if let portIn = testObject.portIn {
            ...
            testResult.set(RESULT_TCP_IN, string: "FAILED") // assume test failed if port was provided (don't have to set failed later, only have to success)
        ...
        if testObject.portOut == nil && testObject.portIn == nil {
            testResult.set(RESULT_TCP_OUT, string: "NOT_SET")
            testResult.set(RESULT_TCP_IN, string: "NOT_SET")
...
// MARK: GCDAsyncSocketDelegate methods
    ...
    @objc func socket(_ sock: GCDAsyncSocket, didRead data: Data, withTag tag: Int) {
        ...
        if sock == tcpTestOutSocket {
            ...
            case TAG_TCPTEST_OUT_PING:
                ...
                testResult.set(RESULT_TCP_RESPONSE_OUT, string: responseWithoutLastNewline)
                testResult.set(RESULT_TCP_OUT, string: "OK")

                // close socket
                //sock.disconnectAfterReadingAndWriting()
                //qosLog.debug("1bef DISCONNECT")
                //sock.disconnectAfterReading()
                sock.disconnect()
                //qosLog.debug("1aft DISCONNECT")
            ...
            testResult.set(RESULT_TCP_RESPONSE_IN, string: responseWithoutLastNewline)
            testResult.set(RESULT_TCP_IN, string: "OK")

            // close socket
            //sock.disconnectAfterReadingAndWriting()
            //qosLog.debug("2bef DISCONNECT")
            sock.disconnect()
            //sock.disconnectAfterReading()
            //qosLog.debug("2aft DISCONNECT")
    ...
    func socketDidDisconnect(_ sock: GCDAsyncSocket, withError err: Error?) {
        qosLog.debug("DISCONNECTED, err: \(err)")
    }
