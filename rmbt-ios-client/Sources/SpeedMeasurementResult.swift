...
class SpeedMeasurementResult: BasicRequest {
    ...
    ///
    var pingShortest: UInt64? {
        get {
            return bestPingNanos
        }
        set {
            // do nothing
        }
    ...
    ///
    var portRemote: UInt16?
    
    ///
    var sourcePortMap = SpeedMeasurementSourcePortMap()
    ...
    ///
    var totalBytesDownload: UInt64?

    ///
    var totalBytesUpload: UInt64?

    ///
    var interfaceTotalBytesDownload: UInt64 = 0

    ///
    var interfaceTotalBytesUpload: UInt64 = 0

    ///
    var interfaceDltestBytesDownload: UInt64 = 0

    ///
    var interfaceDltestBytesUpload: UInt64 = 0

    ///
    var interfaceUltestBytesDownload: UInt64 = 0

    ///
    var interfaceUltestBytesUpload: UInt64 = 0

    ///
    var time: Date?

    ///
    var relativeTimeDlNs: UInt64?

    ///
    var relativeTimeUlNs: UInt64?
    ...
    func setSourcePort(port: Int, direction: SpeedMeasurementSourcePortMap.Direction, forThreadIndex threadIndex: Int) {
        switch direction {
        case .down:
            sourcePortMap.downloadSourcePorts[String(threadIndex)] = port
        case .up:
            sourcePortMap.uploadSourcePorts[String(threadIndex)] = port
        }
        
        //logger.debug(sourcePortMap)
    }
    ...
    func flush() -> [RMBTThroughput]! {
        for h in currentHistories {
            (h as! RMBTThroughputHistory).freeze()
        }

        var result = updateTotalHistory()! // TODO: !
    ...
    override func mapping(map: Map) {
        ...
        pingShortest            <- (map["ping_shortest"], UInt64NSNumberTransformOf)
        portRemote              <- (map["port_remote"], UInt16NSNumberTransformOf)
        sourcePortMap           <- map["source_ports"]
        speedDownload           <- (map["speed_download"], UInt64NSNumberTransformOf)
        speedUpload             <- (map["speed_upload"], UInt64NSNumberTransformOf)

        token                   <- map["token"]
        totalBytesDownload      <- (map["total_bytes_download"], UInt64NSNumberTransformOf)
        totalBytesUpload        <- (map["total_bytes_upload"], UInt64NSNumberTransformOf)
        interfaceTotalBytesDownload  <- (map["interface_total_bytes_download"], UInt64NSNumberTransformOf)
        interfaceTotalBytesUpload    <- (map["interface_total_bytes_upload"], UInt64NSNumberTransformOf)
        interfaceDltestBytesDownload <- (map["interface_dltest_bytes_download"], UInt64NSNumberTransformOf)
        interfaceDltestBytesUpload   <- (map["interface_dltest_bytes_upload"], UInt64NSNumberTransformOf)
        interfaceUltestBytesDownload <- (map["interface_ultest_bytes_download"], UInt64NSNumberTransformOf)
        interfaceUltestBytesUpload   <- (map["interface_ultest_bytes_upload"], UInt64NSNumberTransformOf)

        time              <- map["time"]
        relativeTimeDlNs  <- (map["relative_time_dl_ns"], UInt64NSNumberTransformOf)
        relativeTimeUlNs  <- (map["relative_time_ul_ns"], UInt64NSNumberTransformOf)
