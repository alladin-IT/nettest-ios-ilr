...
open class RMBTMapOptionsType: NSObject {
    ...
    ///
    fileprivate var _paramsDictionary: [AnyHashable: Any]!
    ...
    open func paramsDictionary() -> [AnyHashable: Any] {
        if _paramsDictionary == nil {
            _paramsDictionary = [AnyHashable: Any]() //NSMutableDictionary()

            for f in filters {
                (f.activeValue.info as! [AnyHashable: Any]).forEach({ _paramsDictionary.updateValue($1, forKey: $0)
                })
