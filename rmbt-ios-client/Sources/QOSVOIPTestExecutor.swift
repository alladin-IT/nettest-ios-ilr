...
class QOSVOIPTestExecutor<T: QOSVOIPTest>: QOSTestExecutorClass<T>, UDPStreamSenderDelegate {
    ...
    override func startTest() {
        ...
        testResult.set(RESULT_VOIP_STATUS,          status: .ok)
    ...
    override func executeTest() {
        ...
        // announce voip test
        var voipCommand = "VOIPTEST \(testObject.portOut!) \(testObject.portIn ?? 0) \(testObject.sampleRate) \(testObject.bitsPerSample) "
        ...
        logger.debug("VOIP COMMAND: \(voipCommand)")
        logger.debug("initialSequenceNumber: \(self.initialSequenceNumber)")
    ...
    override func testDidTimeout() {
        testResult.set(RESULT_VOIP_STATUS, status: .timeout)
    ...
    override func testDidFail() {
        testResult.set(RESULT_VOIP_STATUS, status: .error)
...
// MARK: test methods
    ...
    ///
    fileprivate func startOutgoingTest(ssrc: UInt32) {
        ...
        let boolOk = udpStreamSender?.send() ?? false
        ...
        switch tag {
            case TAG_TASK_VOIPTEST:
                ...
                if response.hasPrefix("OK") {
                    ...
                    // check for correct ssrc and fail test otherwise
                    if let ssrc = UInt32(response.components(separatedBy: " ")[1]) {
                        qosLog.info("got ssrc: \(ssrc)")

                        startOutgoingTest(ssrc: ssrc)
                    } else {
                        testDidFail()
                    }
                }
...
// MARK: UDPStreamSenderDelegate methods
    ...
    /// returns false if the class should stop
    func udpStreamSender(_ udpStreamSender: UDPStreamSender, willSendPacketWithNumber packetNumber: UInt16, data: inout Data) -> Bool {
        if packetNumber > 0 {
        ...
        let payloadBytes = malloc(payloadSize) // CAUTION! this sends memory dump to server...

        initialRTPPacket.payload = Data(bytes: payloadBytes! /* TODO: ! */, count: Int(payloadSize)) // TODO: is this correct?
        //initialRTPPacket.payload = Data(bytes: UnsafePointer<UInt8>(&payloadBytes), count: Int(payloadSize))
        free(payloadBytes)

        //

        data.append(initialRTPPacket.toData())
