...
class UDPStreamReceiver: NSObject {
    ...
    ///
    fileprivate let socketQueue = DispatchQueue(label: "at.alladin.rmbt.udp.socketQueue"/*, attributes: DispatchQueue.Attributes.concurrent*/)
    ...
    fileprivate func receivePacket(_ dataReceived: Data, fromAddress address: Data) { // TODO: use dataReceived
        ...
        if settings.sendResponse {
            ...
            if shouldSendResponse && data.count > 0 {
                udpSocket.send(data, toAddress: address, withTimeout: nsToSec(settings.timeout), tag: -1) // TODO: TAG
            }
        }
    }
